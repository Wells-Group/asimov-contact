...
from dolfinx.fem import (Constant, Expression, Function, FunctionSpace, IntegralType, VectorFunctionSpace, locate_dofs_geometrical, locate_dofs_topological)
...
if __name__ == "__main__":
    ...
    if threed:
        ...
        dirichlet_nodes = locate_entities(mesh, 0, lambda x: np.logical_and(np.isclose(x[0], 0), np.logical_or(np.isclose(x[1], 0), np.isclose(x[1], 0.1)) ))
    ...
    sigma_n_h.name = "p"
    u.name = "u"
    with XDMFFile(mesh.comm, f"test_hertz.xdmf", "w") as xdmf:
    ...
    integration_entities, num_local = dolfinx_contact.compute_active_entities(mesh._cpp_object, facet_marker.find(contact_bdy_2),
