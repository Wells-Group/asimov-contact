...
def test_contact_kernel(theta, gamma, dim, gap):
    ...
    for i in refs:
        ...
        F += (
            1
            / gammah
            * R_minus(sigma_n(u) + gammah * (gap - ufl.dot(u, (-n_2))))
            * (theta * sigma_n(v) - gammah * ufl.dot(v, (-n_2)))
            * ds(bottom_value)
        )
        ...
        J += (
            ...
            * 0.5
            * (1 - ufl.sign(q))
            * (sigma_n(du) - gammah * ufl.dot(du, (-n_2)))
            * (theta * sigma_n(v) - gammah * ufl.dot(v, (-n_2)))
            * ds(bottom_value)
        )
        ...
        kernel = dolfinx_contact.cpp.generate_rigid_surface_kernel(V._cpp_object, kt.Jac, q_rule)
        B.zeroEntries()
