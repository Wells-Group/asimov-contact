...
def nitsche_pseudo_time(steps: int, lhs: ufl.Form, rhs: ufl.Form, u: fem.Function, markers: list[_cpp.mesh.MeshTags_int32],
    ...
    vtx = io.VTXWriter(mesh.comm, "pseudo_time_simplex.bp", [u])
    ...
    for tt in range(steps):
        ...
        F_custom = fem.form(F - t * rhs, form_compiler_params=form_compiler_params, jit_params=jit_params)
        J_custom = fem.form(J, form_compiler_params=form_compiler_params, jit_params=jit_params)
        ...
        def compute_coefficients(x, coeffs):
            du.vector[:] = x.array
            u_candidate = []
            u_hat.x.array[:] = du.x.array + u.x.array
            with common.Timer("~~Contact: Pack u contact"):
                for i in range(len(contact_pairs)):
                    u_candidate.append(contact.pack_u_contact(i, du._cpp_object, gaps[i]))
            u0 = []
            u1 = []
            with common.Timer("~~Contact: Pack u"):
                for i in range(len(contact_pairs)):
                    u0.append(dolfinx_cuas.pack_coefficients([du], entities[i]))
                    u1.append(dolfinx_cuas.pack_coefficients([u_hat], entities[i]))
            for i in range(len(contact_pairs)):
                c_0 = np.hstack([coeffs_const[i], u0[i], u1[i], u_candidate[i]])
                coeffs[i][:, :] = c_0[:, :]
