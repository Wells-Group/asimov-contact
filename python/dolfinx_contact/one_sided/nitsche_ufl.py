...
def nitsche_ufl(
    ...
    """Use UFL to compute the one sided contact problem with a mesh coming
    into contact with a rigid surface (not meshed).
    ...
    mesh
        The input mesh
    mesh_data
        A triplet with a mesh tag for facets and values v0, v1. v0
        should be the value in the mesh tags for facets to apply a
        Dirichlet condition on. v1 is the value for facets which should
        have applied a contact condition on
    physical_parameters
    ...
    nitsche_parameters
        ...
        conditions
    plane_loc
        The location of the plane in y-coordinate (2D) and z-coordinate
        (3D)
    ...
    form_compiler_options
        ...
        --help` at the commandline to see all available options. Takes
        priority over all other parameter values, except for
    ...
    J = ufl.inner(sigma(du), epsilon(v)) * ufl.dx - theta / gamma_scaled * sigma_n(du) * sigma_n(
        v
    ) * ds(bottom_value)
    J += (
        1
