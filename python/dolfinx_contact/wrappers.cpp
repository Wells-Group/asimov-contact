...
{
  py::class_<dolfinx_contact::Contact, std::shared_ptr<dolfinx_contact::Contact>>(m, "Contact",
                                                                                  "Contact object")
      ...
      .def("pack_gap_update",
           [](dolfinx_contact::Contact& self, int origin_meshtag,
              const py::array_t<PetscScalar, py::array::c_style>& du)
           {
             auto [coeffs, cstride] = self.pack_gap_update(
                 origin_meshtag, xtl::span<const PetscScalar>(du.data(), du.size()));
             int shape0 = cstride == 0 ? 0 : coeffs.size() / cstride;
             return dolfinx_contact_wrappers::as_pyarray(std::move(coeffs),
                                                         std::array{shape0, cstride});
           })
      ...
      .def("qp_phys_0",
           [](dolfinx_contact::Contact& self, int i)
           {
             ...
             return dolfinx_contact_wrappers::xt_as_pyarray(std::move(self.qp_phys_0()[i]));
           })
      .def("qp_phys_1",
           [](dolfinx_contact::Contact& self, int i)
           {
             ...
             return dolfinx_contact_wrappers::xt_as_pyarray(std::move(self.qp_phys_1()[i]));
           })
      .def("qp_phys_1", &dolfinx_contact::Contact::qp_phys_1)
  ...
  m.def(
         ...
         bool constant_normal)
      {
        return cuas_wrappers::KernelWrapper(
            dolfinx_contact::generate_contact_kernel(V, type, q_rule, coeffs, constant_normal));
      },
