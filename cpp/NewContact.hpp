...
public:
                   ...
                   const basix::FiniteElement& element)
  {
    ...
    candidate_cells.reserve(_surface_1->offsets().back());
    for (std::size_t i = 0; i <= _surface_1->num_nodes(); i++)
      if (_surface_1->num_links(i) > 0)
        candidate_cells.push_back(i);
    const int tdim = _mesh->topology().dim();
    dolfinx::geometry::BoundingBoxTree candidate_bbox(*_mesh, tdim,
                                                      candidate_cells);
    auto candidate_midpoint_tree = dolfinx::geometry::create_midpoint_tree(
        *_mesh, tdim, candidate_cells);

    // Compute number of unique cells on surface_0
    int unique_cells = 0;
    for (std::size_t i = 0; i <= _surface_0->num_nodes(); i++)
      if (_surface_0->num_links(i) > 0)
        unique_cells++;
    ...
    closest_cells.reserve(max_qp * _surface_0->offsets().back());
    ...
    for (std::size_t i = 0; i < _surface_0->num_nodes(); i++)
    {
      auto facets = _surface_0->links(i);
      for (std::size_t j = 0; j < facets.size(); j++)
      {
        ...
        for (std::int32_t j = 0; j < num_q_points; j++)
        {
          ...
          auto [cell0, R0] = dolfinx::geometry::compute_closest_entity(
              candidate_midpoint_tree, point, *_mesh);
          // Find closest facet
