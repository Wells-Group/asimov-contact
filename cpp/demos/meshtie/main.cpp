...
auto read_mesh(const std::string& filename)
{
  ...
  dolfinx::fem::CoordinateElement<double> cmap = dolfinx::fem::CoordinateElement<double>(
  ...
  auto mesh = std::make_shared<dolfinx::mesh::Mesh<double>>(
      dolfinx::mesh::create_mesh(MPI_COMM_WORLD, cells_adj, {cmap},
                                 _x, xshape,
...
int main(int argc, char* argv[])
{
  ...
  loguru::set_thread_name(thread_name.c_str());
  {
    ...
    auto V = std::make_shared<fem::FunctionSpace<double>>(
        fem::create_functionspace(functionspace_form_linear_elasticity_J, "w", mesh));
    auto V0 = std::make_shared<fem::FunctionSpace<double>>(
        fem::create_functionspace(functionspace_form_linear_elasticity_J, "mu", mesh));    
    ...
    lmbda->interpolate(
        [lmbda_val](auto x) -> std::pair<std::vector<double>, std::vector<std::size_t>>
        {
    ...
    mu->interpolate(
        [mu_val](auto x) -> std::pair<std::vector<double>, std::vector<std::size_t>>
        {
          std::vector<double> _f;
          for (std::size_t p = 0; p < x.extent(1); ++p)
          {
    ...
    t->interpolate(
      ...
      auto facet_domains = fem::compute_integration_domains(
        fem::IntegralType::exterior_facet, *facet1.topology(), facet1.indices(), facet1.dim(),
        facet1.values());

    // Define variational forms
    auto J = std::make_shared<fem::Form<double>>(fem::create_form<double>(
        *form_linear_elasticity_J, {V, V}, {{"mu", mu}, {"lmbda", lmbda}}, {}, {}));
    auto F = std::make_shared<fem::Form<double>>(
        fem::create_form<double>(*form_linear_elasticity_F, {V}, {{"f", f}, {"t", t}}, {}, {{dolfinx::fem::IntegralType::exterior_facet, facet_domains}}));

    // Define boundary conditions
    const std::int32_t dirichlet_bdy = 12; // bottom face
    auto facets = facet1.find(dirichlet_bdy);
    auto bdofs = dolfinx::fem::locate_dofs_topological(*V->mesh()->topology_mutable(), *V->dofmap(), 2, facets);
    auto bc = std::make_shared<const dolfinx::fem::DirichletBC<double>>(std::vector<double>({0.0, 0.0, 0.0}), bdofs, V);
    ...
    auto contact_markers = std::make_shared<dolfinx::graph::AdjacencyList<std::int32_t>>(std::move(data), std::move(offsets));
    std::vector<std::shared_ptr<dolfinx::mesh::MeshTags<std::int32_t>>> markers = {std::make_shared<dolfinx::mesh::MeshTags<std::int32_t>>(facet1)};
    std::vector<std::array<int, 2>> pairs = {{0, 1}, {1, 0}};
    auto meshties = dolfinx_contact::MeshTie(markers, contact_markers, pairs, V, 5);

    meshties.generate_meshtie_data(u, lmbda, mu, E*gamma, theta);
    

    // Create matrix and vector
    auto A = dolfinx::la::petsc::Matrix(meshties.create_petsc_matrix(*J, std::string()), false);
    dolfinx::la::Vector<double> b(F->function_spaces()[0]->dofmap()->index_map,
    ...
    dolfinx::fem::apply_lifting<double, double>(b.mutable_array(), {J}, {{bc}}, {}, double(1.0));
    b.scatter_rev(std::plus<double>());
    dolfinx::fem::set_bc<double, double>(b.mutable_array(), {bc});

    // Assemble matrix
    MatZeroEntries(A.mat());
    meshties.assemble_matrix(la::petsc::Matrix::set_block_fn(A.mat(), ADD_VALUES));
    ...
    dolfinx::fem::set_diagonal<double>(dolfinx::la::petsc::Matrix::set_fn(A.mat(), INSERT_VALUES), *V,
                         {bc});
    ...
    MatNullSpace ns = dolfinx_contact::build_nullspace_multibody(*V, domain1, std::vector<std::int32_t>({1, 2}));
    ...
    dolfinx::la::petsc::Vector _u(dolfinx::la::petsc::create_vector_wrap(*u->x()), false);
    dolfinx::la::petsc::Vector _b(dolfinx::la::petsc::create_vector_wrap(b), false);
