...
void dolfinx_contact::MeshTie::pack_coefficients(
    ...
    std::shared_ptr<dolfinx::fem::Function<double>> h, double gamma,
    double theta)
{
    ...
    const std::span<const PetscScalar>& constants)
{
  ...
  assemble_vector(b, pair, kernel, _coeffs[pair], cstride, constants);
}

std::pair<std::vector<double>, std::size_t> dolfinx_contact::MeshTie::coeffs(int pair)
{
    std::size_t cstride = coefficients_size(true);
