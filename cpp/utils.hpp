...
namespace dolfinx_contact
{
                              ...
                              const int q)
  {
    ...
    const std::int32_t num_cells = mesh->topology().index_map(tdim)->size_local() + mesh->topology().index_map(tdim)->num_ghosts();
      ...
      for (std::int32_t cell = 0; cell < num_cells; ++cell)
      {
        ...
        for (std::size_t i = 0; i < dofs.size(); ++i)
        {
          ...
          for (int q = 0; q < num_points; ++q)
            for (int k = 0; k < bs; ++k)
              for (int j = 0; j < vs; j++)
                c[offset + q * (bs * vs) + k + j] += basis_reference_values(q, i, j) * data[pos_v + k];
        }
      }
    }
                         ...
                         const xtl::span<const std::int32_t> &active_facets)
  {
    ...
    if (needs_dof_transformations)
    {
      ...
      for (int facet = 0; facet < num_facets; facet++)
      {
        ...
        // Permute the reference values to account for the cell's orientation
        cell_basis_values = xt::view(basis_reference_values, local_index, xt::all(), xt::all(), xt::all());
        for (std::size_t q = 0; q < num_points; ++q)
        {
          transformation(
              xtl::span(cell_basis_values.data() + q * num_dofs / bs * vs, num_dofs / bs * vs),
        ...
        for (std::size_t i = 0; i < dofs.size(); ++i)
        {
          ...
          for (int q = 0; q < num_points; ++q)
            for (int k = 0; k < bs; ++k)
              for (int l = 0; l < vs; l++)
              {
                c[offset + q * (bs * vs) + k + l] += basis_reference_values(local_index, q, i, l) * data[pos_v + k];
              }
        }
      }
    }
                          ...
                          const xtl::span<const std::int32_t> &active_facets)
  {
    ...
    for (int facet = 0; facet < num_facets; facet++)
    {
      ...
      if (cell_type == "triangle")
      {
        ...
        {
          a += (coordinate_dofs(0, i) - coordinate_dofs(1, i)) * (coordinate_dofs(0, i) - coordinate_dofs(1, i));
          b += (coordinate_dofs(1, i) - coordinate_dofs(2, i)) * (coordinate_dofs(1, i) - coordinate_dofs(2, i));
          c += (coordinate_dofs(2, i) - coordinate_dofs(0, i)) * (coordinate_dofs(2, i) - coordinate_dofs(0, i));
        }
        a = std::sqrt(a);
        b = std::sqrt(b);
        c = std::sqrt(c);
      ...
      else if (cell_type == "tetrahedron")
      {
        ...
        {
          a += (coordinate_dofs(0, i) - coordinate_dofs(1, i)) * (coordinate_dofs(0, i) - coordinate_dofs(1, i));
          b += (coordinate_dofs(0, i) - coordinate_dofs(2, i)) * (coordinate_dofs(0, i) - coordinate_dofs(2, i));
          c += (coordinate_dofs(0, i) - coordinate_dofs(3, i)) * (coordinate_dofs(0, i) - coordinate_dofs(3, i));
          A += (coordinate_dofs(2, i) - coordinate_dofs(3, i)) * (coordinate_dofs(2, i) - coordinate_dofs(3, i));
          B += (coordinate_dofs(1, i) - coordinate_dofs(3, i)) * (coordinate_dofs(1, i) - coordinate_dofs(3, i));
          C += (coordinate_dofs(1, i) - coordinate_dofs(2, i)) * (coordinate_dofs(1, i) - coordinate_dofs(2, i));
        }
        a = std::sqrt(a);
        b = std::sqrt(b);
        c = std::sqrt(c);
        ...
        B = std::sqrt(B);
        C = std::sqrt(C);
        h = std::sqrt((a * A + b * B + c * C) * (a * A + b * B - c * C) * (a * A - b * B + c * C) * (b * B + c * C - a * A)) / (24 * cellvolume);
      }
      // Sum up quadrature contributions
      c[facet] = h;
    }
